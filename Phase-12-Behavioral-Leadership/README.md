# 🎤 PHASE 12: BEHAVIORAL + LEADERSHIP (Week 16)

**Goal**: Tell your technical story effectively

**Learning Objectives**:

- Prepare compelling STAR stories
- Demonstrate technical leadership
- Handle behavioral questions confidently
- Align stories with company values

**Estimated Time**: 10-15 hours

---

## Topics:

### STAR Framework

1. **STAR Method**
   - Situation: Set the context
   - Task: Your responsibility
   - Action: What YOU did (not the team)
   - Result: Quantifiable impact
   - Practice 10+ stories
   - Technical decision stories

### Core Story Types

2. **Project Ownership Stories**

   - "Tell me about a project you owned"
   - End-to-end responsibility
   - Impact metrics
   - Scope and complexity
   - Stakeholder management

3. **Conflict Resolution**

   - Technical disagreements
   - Convincing skeptical stakeholders
   - Compromise examples
   - Handling pushback
   - Maintaining relationships

4. **Technical Decision-Making**

   - Trade-off analysis framework
   - How you evaluate options
   - Data-driven decisions
   - Risk assessment
   - Reversible vs irreversible decisions

5. **Working with Ambiguity**

   - Clarifying vague requirements
   - Starting with incomplete info
   - Iterative approach
   - Dealing with uncertainty
   - Making progress despite unknowns

6. **Failure Stories**
   - Production incidents
   - What you learned
   - How you improved
   - Taking ownership
   - Preventing recurrence

### Leadership

7. **Leadership Principles**

   - Leading without authority
   - Technical leadership
   - Mentoring junior engineers
   - Code review leadership
   - Technical decision ownership
   - Building consensus

8. **Cross-Functional Collaboration**

   - Working with product managers
   - Working with designers
   - Working with other engineering teams
   - Stakeholder communication
   - Managing expectations

9. **Influence & Persuasion**

   - Convincing teams to adopt new tech
   - Technical proposal writing (RFC, ADR)
   - Building consensus
   - Handling pushback
   - Data-driven arguments

10. **Mentoring Stories**
    - Teaching others
    - Code review feedback
    - Onboarding new team members
    - Knowledge sharing
    - Growing others

### Company-Specific Preparation

11. **Amazon Leadership Principles**

    - Customer Obsession
    - Ownership
    - Invent and Simplify
    - Are Right, A Lot
    - Learn and Be Curious
    - Hire and Develop the Best
    - Insist on the Highest Standards
    - Think Big
    - Bias for Action
    - Frugality
    - Earn Trust
    - Dive Deep
    - Have Backbone; Disagree and Commit
    - Deliver Results
    - Prepare 2-3 stories per principle

12. **Google's Googliness**

    - Collaboration
    - Intellectual humility
    - Comfort with ambiguity
    - Taking action
    - Culture fit signals

13. **Meta's Core Values**
    - Move Fast
    - Be Bold
    - Focus on Impact
    - Be Open
    - Build Social Value

### Communication Skills

14. **System Design Communication**

    - How to present designs
    - Handling interruptions
    - Asking clarifying questions
    - Whiteboarding skills
    - Time management in interviews

15. **Explaining Technical Concepts**
    - Adjusting to audience level
    - Using analogies
    - Visual communication
    - Handling "I don't know"
    - Asking for feedback

---

## Story Bank Template:

For each story, prepare:

```
Title: [Short description]
Company/Context: [Where this happened]
Situation: [2-3 sentences of context]
Task: [What was your specific responsibility]
Action: [3-5 specific things YOU did]
Result: [Quantifiable outcome]
Learnings: [What you'd do differently]
LP Mapping: [Which leadership principles it demonstrates]
```

---

## Practice Tips:

1. **Record yourself**: Listen for filler words
2. **Time your stories**: 2-3 minutes each
3. **Practice with a partner**: Get feedback
4. **Prepare follow-ups**: Anticipate deep dives
5. **Be specific**: Names, numbers, timelines

---

## Common Interview Questions:

### Project & Technical

- "Tell me about a project you're proud of"
- "Describe a technical challenge you overcame"
- "Tell me about a time you made a wrong technical decision"
- "How do you prioritize technical debt?"

### Conflict & Collaboration

- "Tell me about a disagreement with a teammate"
- "How do you handle pushback on your ideas?"
- "Describe a time you had to convince others"
- "Tell me about working with a difficult stakeholder"

### Leadership & Growth

- "How have you mentored others?"
- "Tell me about a time you led without authority"
- "How do you stay current with technology?"
- "Describe a time you improved a process"

### Failure & Learning

- "Tell me about a failure"
- "Describe a time you missed a deadline"
- "What's the biggest mistake you've made?"
- "How do you handle criticism?"

---

## Red Flags to Avoid:

- Blaming others
- Taking credit for team work
- Vague, unquantified results
- Not taking ownership
- Speaking negatively about past employers

---

## Deliverable

10 polished STAR stories ready
